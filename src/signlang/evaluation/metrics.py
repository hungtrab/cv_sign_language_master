"""Metric helpers — pure NumPy / pure-Python implementations."""
...
def top_k_accuracy(logits: np.ndarray, labels: np.ndarray, *, k: int = 5) -> float:
    """Logits shape ``(N, C)``, labels shape ``(N,)``."""
...
def confusion_matrix(preds, labels, *, num_classes: int) -> np.ndarray:
    preds = np.asarray(preds, dtype=int); labels = np.asarray(labels, dtype=int)
